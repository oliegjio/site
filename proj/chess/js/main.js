        ...
        function connect_button_event()
        {
            if(in_session == false && input.value != "" && sp_mod == false)
            {
                ...
                $.ajax({
                    ...
                    crossDomain: true,
                    url: "server.php",
                    data: {type: JSON.stringify("new_session"), message: JSON.stringify(message)},
                    success: function(res)
                    {
                        console.log(res);

                        if(res == "too_many_users")
                        {
                            create_session();
        ...
        function disconnect_button_event()
        {
            if(in_session == true && sp_mod == false)
            {
                $.ajax({
                    ...
                    crossDomain: true,
                    url: "server.php",
                    data: {type: JSON.stringify("disconnect"), message: JSON.stringify("disconnect")},
                    success: function(res)
                    {
                        console.log(res);
                    }
        ...
        var ping_interval = setInterval(function()
        {
            if(in_session == true && sp_mod == false)
            {
                $.ajax({
                    ...
                    crossDomain: true,
                    url: "server.php",
                    data: {type: JSON.stringify("ping"), message: JSON.stringify("ping")},
                    success: function(res)
                    {
                        console.log(res);

                        if(res == "enemy_offline")
                        {
                            in_session = false;
                            can_move = false;
                        ...
                        switch(result.type)
                        {
                            ...
                            case "move":
                                ...
                                if(result.message.castling != "false")
                                {
                                    if(result.message.castling == "left")
                                    {
                                        if(result.message.owner == "white_player")
                                        {
                                            map[1][7].piece = "♚";
                                            map[1][7].owner = result.message.owner;
                                            map[3][7].piece = "false";
                                            map[3][7].owner = "false";

                                            map[2][7].piece = "♜";
                                            map[2][7].owner = result.message.owner;
                                            map[0][7].piece = "false";
                                            map[0][7].owner = "false";
                                        ...
                                        {
                                            map[2][0].piece = "♚";
                                            map[2][0].owner = result.message.owner;
                                            map[4][0].piece = "false";
                                            map[4][0].owner = "false";

                                            map[3][0].piece = "♜";
                                            map[3][0].owner = result.message.owner;
                                            map[0][0].piece = "false";
                                            map[0][0].owner = "false";
            ...
            else if(sp_mod == false)
            {
                $.ajax({
                    ...
                    crossDomain: true,
                    url: "server.php",
                    data: {type: JSON.stringify("get_sessions"), message: JSON.stringify("get_sessions")},
                    success: function(res)
                    {
                        console.log(res);
